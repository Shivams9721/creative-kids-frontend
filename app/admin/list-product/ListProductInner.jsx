      ...
      <div className="g2">
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          {step === 2 && (
            <div className="card card-pad">
              ...
              {form.sizes.length > 0 && (
                <>
                  <div style={{ marginTop: 12, fontSize: 11, color: "var(--accent)" }}>✓ {form.sizes.length} sizes: {form.sizes.join(", ")}</div>
                  <div style={{ marginTop: 18 }}>
                    <div className="field-label">SKU by size group</div>
                    <div style={{ display: "grid", gap: 10 }}>
                      {form.sizes.map(size => (
                        <div key={size} style={{ display: "flex", gap: 10, alignItems: "center" }}>
                          <span style={{ minWidth: 110, fontSize: 12, color: "var(--text2)" }}>{size}</span>
                          <input className="field-input" value={form.sku_by_size_group?.[size] || ""}
                            onChange={e => set("sku_by_size_group", { ...form.sku_by_size_group, [size]: e.target.value.toUpperCase() })}
                            placeholder={`${form.sku || "SKU"}-${size.replace(/[^a-zA-Z0-9]/g,"")}`} />
                        </div>
                      ))}
                      <div style={{ fontSize: 11, color: "var(--text3)" }}>Leave empty to use the base SKU for that size.</div>
                    </div>
                  </div>
                </>
              )}
          ...
          {step === 3 && (
            <div className="card card-pad">
              ...
              {form.colors.length > 0 && (
                <div style={{ marginBottom: 18, padding: 12, background: "var(--bg4)", borderRadius: 8, border: "1px solid var(--border)" }}>
                  <div className="field-label" style={{ marginBottom: 10 }}>Base SKU per colour</div>
                  <div style={{ display: "grid", gap: 10 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          <div className="card card-pad">
            ...
            {[["Title",form.title||"—"],["MRP",form.mrp?`₹${form.mrp}`:"—"],["Price",form.price?`₹${form.price}`:"—"],["SKU",form.sku||"—"],["Category",form.main_category||"—"],["Item type",form.item_type||"—"],["Sizes",form.sizes.length?`${form.sizes.length} selected`:"None"],["Colors",form.colors.length?form.colors.join(", "):"None"],["Homepage",form.show_on_homepage ? `Yes / ${form.homepage_section.replace(/_/g, " ")}` : "No"]].map(([k,v]) => (
