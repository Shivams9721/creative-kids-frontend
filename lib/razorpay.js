...
export const processRazorpayPayment = ({ cartItems, couponCode, loyaltyPoints, currency = 'INR', name, email, phone, token, csrfHeaders }) => {
  return new Promise(async (resolve, reject) => {
    try {
      // Step 1: Create Razorpay order on backend.
      // The amount is computed server-side from cart + coupon + loyalty + shipping —
      // the client never sets the price, so a tampered cart cannot be charged at a low amount.
      const orderRes = await safeFetch(`/api/payment/create-order`, {
        ...
        credentials: 'include',
        body: JSON.stringify({ cartItems, couponCode, loyaltyPoints, currency, receipt: `receipt_${Date.now()}` })
      });
