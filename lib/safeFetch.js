const ALLOWED_BASE = process.env.NEXT_PUBLIC_API_URL;
...
export const safeFetch = async (path, options = {}) => {
  ...
  if (options.body && typeof options.body === 'string' && !Object.keys(headers).some(key => key.toLowerCase() === 'content-type')) {
    headers['Content-Type'] = 'application/json';
  }

  const controller = new AbortController();
  const timeoutId = setTimeout(() => controller.abort(), 15000); // 15-second timeout
  const fetchOptions = { ...options, signal: controller.signal, headers };
